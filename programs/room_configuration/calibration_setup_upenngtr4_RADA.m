function [panel, geometry, phantom, objOffset, optimization, data, spAttrb] = calibration_setup_upenngtr4_RADA()
clc;
close all;
clear;
...
panel.invertGray = 1;
panel.factorLow = 20000; % non-normalized 
panel.factorHigh = 65535; % non-normalized
...
data.imagesDir = [data.inputDir,'\RADA']; % directory where images were stored
data.imagesRegExp = 'RADA'; % regular expression to identify images in directory
data.sortBy = 'none'; % 'date'or 'none'
data.anglesFromHeaders = 1;
data.tempDir = [data.inputDir,'\calibration\RADA\6DOF'];
...
spAttrb.imopen_param = 0.4; % image open parameter (to remove clutter)
spAttrb.detectionMode = 'manual'; % detect spheres: auto, manual
