function [geometryOb1, geometryOb2, panelOb1, panelOb2] = roomSetup_testbench_measGEOTOP()
clear all
clc
close all
...
vZ = [0 0 -1];
tablePitch =acosd( vTABn*vZ')/(norm(vTABn,2)*norm(vZ,2))
...
% transformations
srcA_ = TA_ * trans(0,0,SADA);
fpA_ = TA_ * trans(0,0,-AIDA);
% position vectors
srcAPos_ = srcA_(1:3,4);
fpAPos_ = fpA_(1:3,4);

% transformations
srcB_ = TB_ * trans(0,0,SADB);
fpB_ = TB_ * trans(0,0,-AIDB);
% position vectors
srcBPos_ = srcB_(1:3,4);
fpBPos_ = fpB_(1:3,4);
...
% transformations
srcA = TA * trans(0,0,SADA);
fpA = TA * trans(0,0,-AIDA);
% position vectors
srcAPos = srcA(1:3,4)
fpAPos = fpA(1:3,4)

% transformations
srcB = TB * trans(0,0,SADB);
fpB = TB * trans(0,0,-AIDB);
% position vectors
srcBPos = srcB(1:3,4)
fpBPos = fpB(1:3,4)
